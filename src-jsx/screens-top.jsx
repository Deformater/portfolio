  ...
  function Nav({ onNav, active }) {
    ...
    return (
      <header style={{
        ...
        <button onClick={() => onNav('home')} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'baseline', gap: 10 }}>
          ...
          <span style={{ fontFamily: 'var(--font-sans)', fontWeight: 600, fontSize: 17, letterSpacing: '-0.01em', color: 'var(--text-heading)' }}>муравенко</span>
          <span style={{ fontFamily: 'var(--font-serif)', fontStyle: 'italic', fontSize: 17, color: 'var(--text-muted)' }}>engineering</span>
        </button>
  ...
  function Hero({ onOpen, featured }) {
    ...
    return (
      <section data-screen-label="Главная" style={{ position: 'relative', minHeight: '100vh', overflow: 'hidden', background: 'var(--bg)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: 'var(--container)', margin: '0 auto', padding: '120px clamp(20px,5vw,48px) 80px', minHeight: '100vh', boxSizing: 'border-box', display: 'grid', gridTemplateColumns: 'minmax(0,1.45fr) minmax(0,0.62fr)', alignItems: 'center', gap: 'clamp(24px,4vw,56px)' }}>
          {/* ---- frosted panel ---- */}
          <div className="hero-rise" style={{ ...frost, borderRadius: 28, padding: 'clamp(38px,3.4vw,52px) clamp(40px,4vw,60px)' }}>
          ...
          {/* ---- floating glass chips (depth) ---- */}
          <div style={{ position: 'relative', height: '100%', minHeight: 420, pointerEvents: 'none' }}>
            {chips.map((c) => (
              <div key={c.l} className="hero-rise" style={{ ...frost, position: 'absolute', top: c.top, right: c.right, borderRadius: 20, padding: '20px 24px', minWidth: 150, animationDelay: `${c.d}s`, animationDuration: '.9s' }}>
                <div style={{ fontFamily: 'var(--font-sans)', fontWeight: 600, fontSize: 28, letterSpacing: '-0.02em', color: 'var(--text-heading)' }}>{c.n}</div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)', marginTop: 7 }}>{c.l}</div>
              </div>
            ))}
          </div>
        </div>
